...
public class TaskFacadeREST extends AbstractFacade<Task> {
    ...
    public ProjectDummy findMyProjectTasksPOJO() {
        ...
        List<Task> taskList = new ArrayList<Task>();
        Query q = em.createNativeQuery("SELECT DISTINCT task.task_id, task.user_created, task.user_assigned, task.priority, task.start_date, task.end_date, task.start_date_real, task.end_date_real, task.parent_task_id, task.task_title, task.task_description, task.project_id, task.is_personal, task.canceled, task.completed, task.deleted FROM task INNER JOIN project ON task.project_id = project.project_id INNER JOIN project_client ON project.project_id = project_client.project_id INNER JOIN client ON project_client.client_id = client.client_id INNER JOIN client_user ON client.client_id = client_user.client_id where client_user.user_id = ?1 and (task.deleted = false or task.deleted is null) and (project.deleted = false or project.deleted is null)", Task.class);
    ...
    public String findAllTasks(@QueryParam("status") String statusId, @QueryParam("minDate") String minDate, @QueryParam("maxDate") String maxDate, @QueryParam("projectId") String projectId, @QueryParam("clientId") String clientId, @QueryParam("devId") String devId) {
        ...
        if (security.isUserInRole("administrator")) {
            ...
            q = em.createNativeQuery("SELECT DISTINCT task.task_id, task.user_created, task.user_assigned, task.priority, task.start_date, task.end_date, task.start_date_real, task.end_date_real, task.parent_task_id, task.task_title, task.task_description, task.project_id, task.is_personal, task.canceled, task.deleted, task.completed FROM task LEFT OUTER JOIN project ON task.project_id = project.project_id LEFT OUTER JOIN project_client ON project.project_id = project_client.project_id WHERE task.parent_task_id IS NULL and task.is_personal = false and (task.deleted = false or task.deleted is null) and task.task_id > 1025 and (project.deleted = false or project.deleted is null)" + filterQuery, Task.class);
